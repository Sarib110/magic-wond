...
def save_and_plot_data():
    ...
    if num_points > 100:
        ...
        interpolator_x = interp1d(x_original, data_array[:, 0], kind='cubic')
        interpolator_y = interp1d(x_original, data_array[:, 1], kind='cubic')
    ...
    tensor = [0] * 25
    tensor[int(predicted_label)-1] = 1
    print("Predicted Label:", tensor)
    spells = [
    "Alohomora",
    "Avada Kedavra",
    "Expelliarmus",
    ...
    prediction_label.config(text=f"Prediction: {spells[int(predicted_label)-1]}")
